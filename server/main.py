...
#         self._core = RBIRightsEnv(task_level=task_level, deterministic=False)  # deterministic for validator
...
class IndiaDebtEnvironment(Environment[RBIAction, RBIObservation, RBIState]):
    ...
    def __init__(self, task_level: str = TASK_LEVEL):
        super().__init__()
        self._task_level = task_level
        self._core = RBIRightsEnv(task_level=task_level)  # stochastic RL mode — lender resistance varies per step
